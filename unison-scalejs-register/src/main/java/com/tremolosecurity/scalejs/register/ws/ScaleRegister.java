	...
	public void doFilter(HttpFilterRequest request, HttpFilterResponse response, HttpFilterChain chain)
		...
		if (request.getRequestURI().endsWith("/register/config")) {
			...
			if (errors.getErrors().isEmpty()) {
				...
				ArrayList<ApprovalData> approvers = null;
				
				if (scaleConfig.isRequireReason()) {
					params.put(Approval.SEND_NOTIFICATION, "false");
					
					
					ApprovalData approver = new ApprovalData();
					approver.setApprover(uid);
					approver.setReason(newUser.getReason());
					
					approvers = new ArrayList<ApprovalData>();
				...
				try {
					exec.execute(wfcall, GlobalEntries.getGlobalEntries().getConfigManager(), approvers);
				} catch(Exception e) {
